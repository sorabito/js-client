...
JR.from('XBT').to('USD').amount(1).convert(function(res) {
    QUnit.test('Converting_One_XBT_To_USD', function(assert) {
        assert.strictEqual(
            res > 100,
            true,
            'Value is greater than hundred => response: ' + res
        );
        assert.strictEqual(
            res.indexOf('.') > -1,
            true,
            'Value contains a dot => response: ' + res
        );
    });
});
...
/** ============================== Test ============================== */
JR.from('JPY').to('SAR').dateStart('2015-01-01').dateEnd('2015-01-03').historical(function(res) {
    QUnit.test('Historical_Rates_For_Three_Days_Gets_Three_Result_Objects', function(assert) {
        assert.strictEqual(
